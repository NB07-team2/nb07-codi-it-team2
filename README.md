...
- **디자인 시안 (Figma):** [바로가기](https://www.google.com/search?q=https://www.figma.com/design/SHHlFNOr1FypN3qrgaLOyT/-%EA%B3%B5%EC%9C%A0%EC%9A%A9--CODI-IT-%ED%8C%A8%EC%85%98-%EC%9D%B4%EC%BB%A4%EB%A8%B8%EC%8A%A4-%ED%94%8C%EB%9E%AB%ED%8F%BC)
...
| **이름** | **담당 역할** | **주요 구현 기능** |
| --- | --- | --- |
| **이보라 (Leader)** | **AWS & Git & 이미지 S3 업로드, Store API** | 스토어 CRUD, AWS S3 이미지 업로드 유틸리티 |
| **김관규** | **장바구니, 대시보드, 알림 API** | 장바구니 퍼시스턴스, 판매 통계 대시보드, 실시간 알림 |
| **구창민** | **구매( 주문 ), 문의, 등급 값 조회 API** | 가상 결제 프로세스, 포인트/등급 산정 엔진, 상품 문의 시스템 |
| **김승철** | **인증, 유저 API** | JWT 기반 로그인 / 회원가입, User CRUD 관리, S3 이미지 라이프 사이클 관리 |
| **하동우** | **상품, 리뷰 API** | 카테고리별 상품 필터링/정렬 시스템, 사용자 별점 및 리뷰 CRUD |
