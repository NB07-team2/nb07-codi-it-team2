...
export async function createInquiry(req:Request, res: Response) {
   ...
   const validatedData = inquiryCreateSchema.parse(dateToValidate); // Zod를 사용하여 데이터 검증
   const newInquiry = await productsService.createInquiry(validatedData); // 실제로는 인증된 사용자 ID와 문의 대상 상품 ID를 가져와야 합니다.
